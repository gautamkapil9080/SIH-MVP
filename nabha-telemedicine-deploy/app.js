...
function checkEligibility(scheme) {
    ...
    switch(scheme) {
        case 'ayushman':
            message = 'To check Ayushman Bharat eligibility, visit: https://pmjay.gov.in/about/pmjay or call 14555';
            break;
        case 'punjab':
            message = 'All Punjab residents are eligible for Punjab Health Scheme. Visit your nearest PHC/CHC.';
            break;
        default:
            message = 'Please contact the relevant helpline for eligibility details.';
    }
    showStatusMessage(message, 'info');
}

function findNearbyCenter(scheme) {
    let message = 'Finding nearby centers for ' + scheme + '... Please contact local health office at 01762-230345';
    showStatusMessage(message, 'info');
}

function findJanAushadhiStore() {
    showStatusMessage('Jan Aushadhi stores: Nabha Main Market, Civil Hospital Nabha', 'info');
}

// Essential utility functions that are missing
...
function showStatusMessage(message, type = 'info') {
    ...
    if (statusDiv) {
        ...
        // Fallback to alert if statusMessage div not found
        alert(message);
    }
}
...
// Load sample medicines if none exist
function loadSampleMedicines() {
    const existingMedicines = getStoredData(STORAGE_KEYS.MEDICINES);
    if (!existingMedicines || existingMedicines.length === 0) {
        const sampleMedicines = [
            { id: generateId(), name: 'paracetamol', availability: 'available', pharmacy: 'Nabha Medical Store', timestamp: new Date().toISOString() },
            { id: generateId(), name: 'amoxicillin', availability: 'limited', pharmacy: 'Civil Hospital Pharmacy', timestamp: new Date().toISOString() },
            { id: generateId(), name: 'omeprazole', availability: 'out_of_stock', pharmacy: 'Nabha Medical Store', timestamp: new Date().toISOString() },
            { id: generateId(), name: 'cough syrup', availability: 'available', pharmacy: 'Patel Pharmacy', timestamp: new Date().toISOString() },
            { id: generateId(), name: 'aspirin', availability: 'available', pharmacy: 'Civil Hospital Pharmacy', timestamp: new Date().toISOString() }
        ];
        localStorage.setItem(STORAGE_KEYS.MEDICINES, JSON.stringify(sampleMedicines));
    }
}
...
function syncOfflineData() {
    ...
    console.log('Syncing offline data...');
    showStatusMessage('Data synced!', 'success');
}
...
function showSymptomResult(resultKey) {
    ...
    if (!result) {
        resultDiv.innerHTML = '<p>Sorry, no specific advice found for your symptoms. Please consult a doctor.</p>';
        return;
    }
    ...
    // Show result
    resultDiv.innerHTML = `
        <div class="symptom-result ${result.urgency}">
            <h3>${result.title}</h3>
            <div class="advice">${result.advice}</div>
            <div class="urgency-level urgency-${result.urgency}">
                <strong>Priority Level: ${result.urgency.toUpperCase()}</strong>
            </div>
            <div class="result-actions">
                <button onclick="restartSymptomChecker()" class="restart-btn">Start Over</button>
                ${result.urgency === 'emergency' ? '<button onclick="callAmbulance()" class="emergency-btn">🚨 Call 108</button>' : ''}
            </div>
        </div>
    `;
}
...
function filterRequests() {
    ...
    if (searchFilter) {
        filteredRequests = filteredRequests.filter(req => 
            req.name.toLowerCase().includes(searchFilter) || 
            req.village.toLowerCase().includes(searchFilter)
        );
    }
...
function displayFilteredRequests(requests) {
    ...
    if (requests.length === 0) {
        requestsList.innerHTML = `<p>No requests found matching your filters.</p>`;
        return;
    }
...
function addMedicineToPrescription() {
    ...
    newMedicine.innerHTML = `
        ...
        <button type="button" onclick="removeMedicineFromPrescription(this)" class="remove-medicine">Remove</button>
    `;
    medicinesList.appendChild(newMedicine);
}

// Remove medicine from prescription form
function removeMedicineFromPrescription(button) {
    const medicineDiv = button.closest('.medicine-prescription');
    medicineDiv.remove();
}
...
function displayPatientHistory(patients, consultations) {
    ...
    if (patients.length === 0) {
        historyResults.innerHTML = '<p>No patients found.</p>';
        return;
    }
    ...
    historyResults.innerHTML = patients.map(patient => {
        ...
        return `
            <div class="patient-history-card">
                <h4>${patient.name} - ${patient.village}</h4>
                <p><strong>Phone:</strong> ${patient.phone} | <strong>Age:</strong> ${patient.age}</p>
                <p><strong>Last Visit:</strong> ${new Date(patient.timestamp).toLocaleDateString()}</p>
                <p><strong>Symptoms:</strong> ${patient.symptoms}</p>
                ${patientConsultations.length > 0 ? `
                    <div class="consultation-history">
                        ...
                        ${patientConsultations.map(consultation => `
                            <div class="consultation-item">
                                <p><strong>Date:</strong> ${new Date(consultation.timestamp).toLocaleDateString()}</p>
                                <p><strong>Doctor:</strong> ${consultation.doctorName}</p>
                                <p><strong>Notes:</strong> ${consultation.response}</p>
                            </div>
                        `).join('')}
...
// Quick symptom check
function quickSymptomCheck(category) {
    const resultDiv = document.getElementById('quickSymptomResult');
    let advice = '';
    
    switch(category) {
        case 'respiratory':
            advice = 'For breathing issues: Rest, drink warm fluids, use steam inhalation. If severe breathing difficulty, call 108 immediately.';
            break;
        case 'digestive':
            advice = 'For stomach problems: Eat light foods, stay hydrated, rest. If severe pain or vomiting blood, seek immediate medical help.';
            break;
        case 'neurological':
            advice = 'For headaches/dizziness: Rest in quiet, dark room, stay hydrated. If sudden severe symptoms, seek immediate medical attention.';
            break;
        case 'cardiovascular':
            advice = 'For chest pain: If severe chest pain or pressure, call 108 immediately. For mild discomfort, rest and monitor.';
            break;
        case 'musculoskeletal':
            advice = 'For joint/muscle pain: Rest, apply ice/heat, gentle movement. If severe or persistent pain, consult a doctor.';
            break;
        case 'skin':
            advice = 'For skin issues: Keep clean and dry, avoid irritants. If spreading rash or severe reaction, see a doctor.';
            break;
        case 'mental_health':
            advice = 'For mental health: Talk to someone you trust, maintain routine, get adequate sleep. If severe distress, seek professional help.';
            break;
        case 'general':
            advice = 'For general symptoms: Rest, stay hydrated, monitor symptoms. If worsening or concerning, consult healthcare provider.';
            break;
        default:
            advice = 'Please use the guided assessment for more specific advice.';
    }
    
    resultDiv.innerHTML = `
        <div class="quick-result">
            <h4>${category.charAt(0).toUpperCase() + category.slice(1)} Health Advice</h4>
            <p>${advice}</p>
            <button onclick="showSection('symptoms')" class="btn">Use Detailed Assessment</button>
        </div>
    `;
}
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('Nabha Telemedicine - Initializing application...');
    ...
    console.log('Nabha Telemedicine - Application ready!');
});
